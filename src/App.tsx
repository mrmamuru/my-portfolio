  ...
  return <div className="cinematic-site">
    ...
    <main>
      ...
      <section id="create" className="section create">
        ...
        <div className="create-grid">{creations.map(item => <motion.article {...reveal} key={item.number} className={`create-card${item.title === 'Filmmaking' ? ' create-card--filmmaking' : ''}`} style={{'--card-image':`url(${item.image})`} as React.CSSProperties}><div className="create-card__media"/><span>{item.number}</span><div><h3>{item.title}</h3><ul>{item.tags.map(tag=><li key={tag}>{tag}</li>)}</ul></div>{item.title === 'Content' ? <a className="create-card__open" href="https://www.instagram.com/mr.mamuru/" target="_blank" rel="noreferrer" aria-label="Explore Content on Instagram (opens in a new tab)"><ArrowUpRight/></a> : <button className="create-card__open" onClick={() => { if (item.title === 'Filmmaking') setFilmmakingOpen(true); else setActiveProject(projects[item.title === 'Production' ? 0 : 1]); }} aria-label={'Explore ' + item.title + ' projects'}><ArrowUpRight/></button>}</motion.article>)}</div>
      </section>
